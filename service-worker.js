const CACHE_NAME='baby-growth-v1-20260720-r29';
const APP_SHELL=[
  ...
  "./src/features/meals/planner.js","./src/features/meals/preferences.js","./src/features/meals/presentation.js","./src/features/meals/recipe-details.js","./src/features/meals/shopping.js","./src/features/meals/week-menu.js","./src/features/meals/menu-browser.js",
  "./src/features/records/records.js","./src/features/records/sleep.js","./src/features/records/new-food.js",
  "./src/features/growth/timeline.js","./src/features/growth/chart.js","./src/features/growth/daily-trends.js","./src/features/growth/latest-summary.js","./src/features/backup/backup.js","./src/features/backup/validate.js","./src/features/migration/v2.js","./src/features/notifications/notifications.js",
...
async function navigationResponse(request){try{const response=await fetch(request);if(response.ok){const cache=await caches.open(CACHE_NAME);cache.put('./index.html',response.clone())}return response}catch{const cache=await caches.open(CACHE_NAME);return cache.match('./index.html')}}
async function assetResponse(request){const cache=await caches.open(CACHE_NAME),cached=await cache.match(request);if(cached)return cached;const response=await fetch(request);if(response.ok)cache.put(request,response.clone());return response}
