...
test('service worker has safe caching, navigation fallback and controlled updates', async () => {
  ...
  assert.ok(sw.includes("const CACHE_NAME='baby-growth-v1-20260720-r29'"));
  ...
  const appFiles=['./','./index.html','./assets/styles/app.css','./data/recipes.js','./data/stage4-recipes-v2.js','./data/stage4-recipes-v2-rows.js','./src/app.js','./src/features/growth/latest-summary.js','./src/features/meals/recipe-details.js','./src/features/migration/v2.js','./src/ui/daily-trends.js','./manifest.webmanifest'];
...
test('service worker activation deletes only obsolete caches owned by this application', async () => {
  ...
  const caches={
    keys:async()=>['baby-growth-v1-20260720-r28','baby-growth-v1-20260720-r29','other-app-cache'],
  ...
  await activation;
  assert.deepEqual(deleted,['baby-growth-v1-20260720-r28']);
});
...
test('service worker reads only the current application cache', async () => {
  ...
  const caches={
    open:async name=>{assert.equal(name,'baby-growth-v1-20260720-r29'); return activeCache},
