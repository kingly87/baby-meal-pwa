...
export function parseAndValidateBackup(text) {
  ...
  for(const item of data.weeklyMenus)if(!Array.isArray(item.days)||item.days.some(entry=>!entry||typeof entry!=='object'||!day(entry.date)||!Array.isArray(entry.meals)||entry.meals.some(meal=>!meal||typeof meal!=='object'||typeof meal.id!=='string'||typeof meal.name!=='string'||!['planned','eaten','skipped'].includes(meal.status)||meal.mealType!==undefined&&!['breakfast','lunch','dinner'].includes(meal.mealType))))fail('weeklyMenus');
