...
function barChart(model,definition){
  ...
  const bars=model.points.map((point,index)=>{
    ...
    return`<g class="trend-bar" tabindex="0" role="img" aria-label="${esc(accessible)}" data-missing="${!point.hasData}"><title>${esc(accessible)}</title><rect class="trend-hit-target" x="${x}" y="0" width="44" height="214"/><text class="trend-value" x="${x+22}" y="${point.hasData?valueY:24}">${esc(visibleValue)}</text><rect class="trend-bar-track" x="${x+13}" y="${barTop}" width="18" height="${barHeight}" rx="7"/>${point.hasData?`<rect class="trend-bar-fill" x="${x+13}" y="${fillY}" width="18" height="${height}" rx="7"/>`:`<rect class="trend-missing-mark" x="${x+13}" y="${barTop}" width="18" height="${barHeight}" rx="7"/>`}<text class="trend-date" x="${x+22}" y="202">${esc(compactDate(label))}</text></g>`;
  }).join('');
