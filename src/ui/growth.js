...
export function growthView({timeline=[],chart,trend,trendError='',measurements=[],teeth=[]}={}){
  const latest=latestGrowthSummary({measurements,teeth}),summary=(item,value,fallback)=>item?`<strong>${esc(value(item.value))}</strong><time>${esc(item.date)}</time>`:`<strong>${fallback}</strong>`;
