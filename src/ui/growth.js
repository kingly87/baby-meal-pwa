...
import{latestGrowthSummary}from'../features/growth/latest-summary.js';
...
export function growthView({timeline=[],chart,trend,trendError='',measurements=[],teeth=[]}={}){
  const latest=latestGrowthSummary({measurements,teeth}),summary=(item,value,fallback)=>item?`<strong>${esc(value(item.value))}</strong><time datetime="${esc(item.date)}">${esc(item.date)}</time>`:`<strong>${fallback}</strong>`;
  const trendSection=trend?`<section id="daily-trends" class="panel"><div class="section-heading"><h2>生活趋势</h2></div>${dailyTrendChart(trend)}</section>`:trendError?`<section id="daily-trends" class="panel"><div class="section-heading"><h2>生活趋势</h2></div><p class="empty" role="status">${esc(trendError)}</p></section>`:'';
  return`<div class="page-stack"><section class="page-title"><p class="eyebrow">成长轨迹</p><h2>慢慢长大的每一天</h2></section><div class="feature-grid"><button class="summary-card action" data-growth="weight"><span class="label">体重</span>${summary(latest.weight,value=>`${value} kg`,'添加记录')}</button><button class="summary-card action" data-growth="height"><span class="label">身高</span>${summary(latest.height,value=>`${value} cm`,'添加记录')}</button><button class="summary-card action" data-growth="tooth"><span class="label">长牙</span>${summary(latest.tooth,value=>`第${value}颗`,'记录一颗牙')}</button></div>${trendSection}<section class="panel"><div class="section-heading"><h2>成长曲线</h2><select id="chart-type"><option value="weight">体重</option><option value="height">身高</option></select></div><div id="growth-chart">${chartSvg(chart)}</div></section><section class="panel"><div class="section-heading"><h2>完整时间线</h2><select id="timeline-filter"><option value="all">全部</option><option value="growth">成长</option><option value="tooth">长牙</option><option value="new-food">新食材</option><option value="sleep">睡眠</option><option value="task">作息事项</option><option value="milk">喝奶</option><option value="water">喝水</option></select></div><div class="timeline timeline-scroll">${timeline.map(item=>`<article data-type="${esc(item.type)}"><time>${esc(formatTimelineDate(item.date))}</time><p>${esc(item.title)}</p></article>`).join('')||'<div class="empty">还没有成长记录。</div>'}</div></section></div>`;
}
